...
// 単語例文：例文でその語が実際に使われているかの判定（語形変化を緩く許容）
const wordBases = (t) => {
  const f = [t]
...
WORD_SENTENCES.forEach((s, i) => {
  ...
  // 文末記号の対応（英 . → 和 。 / 英 ? → 和 ？）
  const expectJa = PUNCT_MAP[s.en.slice(-1)]
  if (expectJa) {
    if (s.ja.slice(-1) !== expectJa) err(`文末記号不一致: 英文末なら和文末は "${expectJa}"`)
...
console.log(`✓ 検証OK（${WORDS.length}単語・${DICT.length}英英・${WORD_SENTENCES.length}例文・エラーなし）`)
