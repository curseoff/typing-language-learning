...
const endNode = (
  <EndConditionSelect kinds={kinds} kind="time" value={60} values={[30, 60, 120]}
    valueLabel={valueLabel} focusSection="" onChange={noop} onChangeValue={noop} onFocusSection={noop} />
);
const words = [
...
const shared = {
  levels, themes, modeGroups, level: 1, theme: 'すべて', mode: 'both',
  onLevelChange: noop, onThemeChange: noop, onModeChange: noop, onFocusSection: noop,
  modeDesc: '英語と日本語を交互に入力します。', poolCount: '収録 320 語',
  onStart: noop, onBottomTabChange: noop, endConditionNode: endNode,
};
export const WordsList = () => (
  <RankSectionView {...shared} focusSection="mode" bottomTab="list"
    browseNode={<ItemList items={words} type="words" mode="both" />} />
);
export const WordsRecords = () => (
  <RankSectionView {...shared} focusSection="level" bottomTab="records"
    browseNode={<RecordsTable records={recs} modeKey="both" rankText="単語 L1 / すべて" endCondition={{ kind: 'time', value: 60 }} />} />
);
