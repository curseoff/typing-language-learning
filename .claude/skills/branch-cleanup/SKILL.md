---
name: branch-cleanup
description: マージ済みブランチの片付け手順。PR をマージした直後、`git branch` にブランチが溜まっているとき、どれを消してよいか判断したいときに読む。削除して良いかの判定方法（develop/master 両方への到達確認）と、消してはいけない例外ブランチ（`issue-assets`・`feature/srs-review`）を含む。
---
