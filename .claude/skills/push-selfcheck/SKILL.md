---
name: push-selfcheck
description: 未push差分に公開して問題があるもの（APIキー/トークン/秘密鍵・メールや氏名などの個人情報・絶対パスでの username 露出・.env/.pem 等の鍵ファイル）が混ざっていないかを1コマンドで点検する。push する前、PR を作る前、「この差分は公開して大丈夫か」を確認したいときに使う。手で grep を組み立てる代わりにこれを使う。
---
