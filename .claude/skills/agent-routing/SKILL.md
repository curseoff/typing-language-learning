---
name: agent-routing
description: 変更をどのサブエージェントへ委任するかの詳細規約（各役の守備範囲・起動タイミング・境界の切り分け）。CLAUDE.md の役割表で役を選んだ後、その役に何をどこまで任せるかを確認するときに読む。委任先の判断に迷ったとき、役どうしの守備範囲が重なって見えるとき、委任ブリーフを書くとき、並列運用（worktree 隔離）を検討するときに使う。
---
