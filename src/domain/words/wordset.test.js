...
describe('buildWordSet (4択用)', () => {
  it('指定数の語を返す', () => {
    expect(buildWordSet(1, 'すべて').length).toBe(WORD_COUNT)
    expect(buildWordSet(2, '旅行', 10).length).toBe(10)
  })
})
  ...
  it('最短綴りで打っても600キーに到達できる（短い綴りで詰む不具合の回帰）', () => {
    for (const mode of MODES) {
      for (const level of LEVELS) {
        const words = buildWordPassage(level, 'すべて', mode)
...
describe('makeQuiz (4択)', () => {
  it('英語訳/日本語訳とも、選択肢に正解を含み前方一致が衝突しない', () => {
    for (const dir of ['en', 'ja']) {
      const qs = makeQuiz(buildWordSet(2, 'すべて'), levelWords(2), dir)
      for (const q of qs) {
