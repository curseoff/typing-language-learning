...
- **アーキは arm64 に統一**：GitHub Actions も arm64 ランナー（`ubuntu-24.04-arm`）で動かしており、`check:ci` も**ホストネイティブ（Apple Silicon=arm64）既定**。これで CI とローカルが**完全一致**し、**Rosetta（将来廃止予定）に非依存**。成果物は静的JSでアーキ非依存のため x64 を捨てるリスクは無い。
  - x64 を厳密に確認したいときだけ `CHECK_CI_ARCH=amd64 npm run check:ci`（Rosetta/QEMU が要る）。
...
- ホストの `node_modules` を壊さないよう、コンテナ内 `node_modules` は **tmpfs** に分離している。
...
| 復習（SRS） | `srs-v1` / `srs-meta-v1` | 単語(en)ごとの間隔反復カード `{box,due,reps,lapses}`／日次の新規導入数 |

各ランキングは速い順（4択は正解数順）で最大 15 件（`MAX_RECORDS`）。タッチタイピングは記録を保存しません。

**復習モード（SRS）**：英単語をタイプする産出想起。**デッキ3種**（`application/reviewDecks.js`）＝単語(和訳→en)／英英(定義→見出し語)／単語例文(和文→その語)。SRSは `w:`/`d:`/`s:` 接頭辞で別管理。正解で box を上げ間隔を延ばし（`domain/srs`）、ミスで近く再出題。新規は1日10件まで。各タブ（単語/単語例文/英英）の「🔁 復習する」ボタンで開始。**穴埋めヒント**＝box（習熟）が上がるほど見せる文字が減り（`clozeShown`）、最後は全伏せ＝完全想起。伏せ位置は語ごとに決定的。
