...
**復習モード（SRS）**：英単語をタイプする産出想起。**デッキ3種**（`application/reviewDecks.js`）＝単語(和訳→en)／英英(定義→見出し語)／単語例文(和文→その語)。SRSは `w:`/`d:`/`s:` 接頭辞で別管理。正解で box を上げ間隔を延ばし（`domain/srs`）、ミスで近く再出題。新規は1日10件まで。TOPの「復習」タブでデッキを選んで開始。
