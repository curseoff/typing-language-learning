...
└─ src/
   ...
   └─ ui/                          プレゼンテーション
      ...
      ├─ ready/Ready.jsx           スタート画面（種類タブ＋文脈表示）
...
`App.jsx` が `phase`（ready / playing / story / words / dict / result）と `gameType`（marathon / story / words / dict）を持ち、種類に応じてビューへルーティングします。各 `useXxx` フックが該当ゲームの状態機械（出題・打鍵判定・計測・記録保存）を担います。
