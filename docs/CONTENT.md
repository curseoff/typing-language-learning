...
2. **マージ**：全TSVを連結 → `npm run add-words <連結.tsv>` で検査 → `-- --write` で追記。毎回 `npm run validate` / `npm run check`。
...
#       要素: { word, level, en, ja, kana, jaWords }（カタカナ長音は読みも「ー」で表す＝ケーキ→けーき）
...
検証は構造（`word ∈ words.en` / 例文で使用 / level一致 / `jaWords`連結=ja / kana整合 / 文末記号）を機械チェック。読みの妥当性だけ点検エージェントが判断する。
