...
- **モダンブラウザ**（Chrome / Edge / Safari / Firefox の最新版）で動きます。記録の保存には OPFS（Origin Private File System）対応が必要なので、なるべく新しいブラウザを使ってください。
- キーボード配列は **US・JIS どちらでもOK**です（記号は画面の表示どおりに打てば入力できます）。
- **登録不要・無料**で、ブラウザだけで動きます（サーバーへの送信やログインはありません）。
...
- 記録（速度・ミス・ランキング等）はすべて**あなたのブラウザ内（SQLite/OPFS）に保存**されます。**サーバーへは送信されません**（アカウント不要・無料・広告なし）。
