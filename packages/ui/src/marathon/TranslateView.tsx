...
interface TrSeg {
  ...
  en: string
  kana?: string
  chips: { text: string; i: number; kana?: string }[]
}
...
export default function TranslateView({ segments, segIndex, segInput, hasError }: TranslateViewProps) {
  const seg = segments[segIndex]
  if (!seg) return null
  const toEnglish = seg.type === 'en' // 英訳(和文→英語)
  const sourceOf = (s: TrSeg) => (s.type === 'en' ? s.ja : s.en)
  const next = segments[segIndex + 1]

  // 英訳モードの原文は日本語なのでルビ(ふりがな)を付ける。和訳モードの原文は英語なのでそのまま。
  const renderSource = (s: TrSeg) =>
  ...
  const { used, curDone, curKanaDone } = chipProgress(seg, segInput) // チップ着色用の進捗
