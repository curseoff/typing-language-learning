...
export default defineConfig(({ command }) => ({
  ...
  test: {
    ...
    coverage: {
      ...
      // #468 テストの決定性：テスト時の Math.random をシード付き列へ固定し（src/test/setup.js）、
      // さらに「終了時に入力途中の語がある」partial 分岐を明示的に踏むテストを追加した。これで
      // 出題順まかせだった被覆の揺れが無くなり、実測は実行間で完全一致する（3回連続で同値を確認：
      // S 4986/5531=90.14 / B 3847/4539=84.75 / F 1223/1343=91.06 / L 4165/4557=91.39）。
      // 揺れが消えたので、これまで確率的な閾値割れを避けるために積んでいたマージンは不要になった
      // （それ自体が #468 の CI ランダム失敗の原因）。実測直下へラチェット（up-only）。
      thresholds: { statements: 90.1, branches: 84.7, functions: 91.0, lines: 91.35 },
    },
  },
}))
